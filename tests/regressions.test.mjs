...
test('board pages share filter primitives and flat finder starts from a regional country', () => {
  ...
  for (const source of [flat, hiring, jobs]) {
    ...
    assert.match(source, /<SearchFilterBlocks/u)
  }
...
test('flat finder renders a collapsible localized statistics panel', () => {
  ...
  assert.match(stats, /statistics:\s*FlatStatistics/u)
  assert.match(stats, /statistics\.quality\.duplicatesRejected/u)
  const ru = JSON.parse(readFileSync(new URL('../i18n/locales/ru.json', import.meta.url), 'utf8'))
...
test('all three listing boards use the shared Chart.js analytics components', () => {
  ...
  assert.match(flatStats, /statistics\.ownership/u)
...
test('vacancy cards open from the card surface and compact overflowing pills', () => {
  ...
  assert.match(card, /<UiDraggablePills/u)
  assert.match(pills, /@pointerdown\.stop="startDrag"/u)
  assert.match(pills, /draggable-pills__more[^>]*>\+\{\{/u)
