...
Private Telegram bot for Personal Site search updates:
...
The service is included in the root `docker-compose.yml` and reads the same `${SITE_ENV_FILE:-./db.env}` as the site. The container is deployed in a safe disabled state by default. To activate it add:
