...
import type { Job, JobStats, JobSalaryTrendPoint } from "~/types/jobs";
import { locationLabel } from "~/utils/locationLabels";
import { canonicalCityValue } from "~~/shared/locationCatalog";

const props = defineProps<{
  // Kept for call-site compatibility; trend data now comes from stats.salaryTrend,
  // which is built over the complete filtered result instead of the current page.
  jobs?: Job[];
  stats: JobStats;
...
function groupKey(point: JobSalaryTrendPoint): string {
  ...
  if (trendScope.value === "position") return point.title || t("trendUnknown");
...
@media(max-width:900px){.stats__grid{grid-template-columns:repeat(2,minmax(0,1fr))}.stats__card_wide{grid-column:1/-1}.trends__filters{flex-direction:column}.trends__segments_scope{justify-content:flex-start}}
