...
<template>
  <div v-if="photos.length" class="flat-gallery">
    <img
...
<style scoped>
.flat-gallery { display: grid; grid-template-columns: repeat(auto-fill, minmax(120px, 1fr)); gap: 6px; max-height: 46vh; overflow-y: auto; }
.flat-gallery__thumbnail { width: 100%; aspect-ratio: 4 / 3; object-fit: cover; border-radius: 8px; cursor: zoom-in; border: 1px solid var(--line); transition: border-color 140ms ease; }
.flat-gallery__thumbnail:hover { border-color: var(--accent-pink); }
