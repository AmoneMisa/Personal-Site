...
export function useFlatFilters() {
  ...
  // from north (270 = due west). Both unset means the whole circle. Membership
  // is evaluated by the backend/database; the browser only edits/draws the arc.
  ...
  function buildFeedParams(options: {
    ...
    // Geographic membership is database-owned. Send the complete station union,
    // radius and optional directional arc before count/dedupe/pagination/map.
    const selectedMetros = [...new Set(metro.value.map((name) => name.trim()).filter(Boolean))];
    if (selectedMetros.length) {
      params.metro = selectedMetros.join(",");
      if (metroMaxM.value != null) params.metroMaxM = String(metroMaxM.value);
      if (metroBearingFrom.value != null && metroBearingTo.value != null) {
        params.metroArc = `${metroBearingFrom.value},${metroBearingTo.value}`;
      }
    }
