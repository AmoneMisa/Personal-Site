...
function normalizeResponse(value: unknown): FlatGeoZonesResponse {
  ...
  return {
    ...
    regionZones: normalizeGroup(raw.regionZones),
    microdistrictMarkers: normalizeGroup(raw.microdistrictMarkers),
    mahallaMarkers: normalizeGroup(raw.mahallaMarkers).length ? normalizeGroup(raw.mahallaMarkers) : normalizeGroup(raw.quartalMarkers),
    quarterMarkers: normalizeGroup(raw.quarterMarkers),
    quartalMarkers: normalizeGroup(raw.quartalMarkers).length ? normalizeGroup(raw.quartalMarkers) : normalizeGroup(raw.mahallaMarkers),
    areaZones: normalizeGroup(raw.areaZones),
    zoneMarkers: normalizeGroup(raw.zoneMarkers).length ? normalizeGroup(raw.zoneMarkers) : normalizeGroup(raw.areaZones),
...
export function useGeoCityCatalog(
  ...
  const allZones = computed(() => [
    ...
    ...data.value.regionZones,
    ...data.value.microdistrictMarkers,
    ...data.value.mahallaMarkers,
    ...data.value.quarterMarkers,
    ...data.value.zoneMarkers,
  ...
  return {
    ...
    regionZones: computed(() => data.value.regionZones),
    microdistrictMarkers: computed(() => data.value.microdistrictMarkers),
    mahallaMarkers: computed(() => data.value.mahallaMarkers),
    quarterMarkers: computed(() => data.value.quarterMarkers),
    quartalMarkers: computed(() => data.value.quartalMarkers),
    areaZones: computed(() => data.value.areaZones),
    zoneMarkers: computed(() => data.value.zoneMarkers),
