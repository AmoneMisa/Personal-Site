...
function jobRow(job: Job, syncToken: string) {
  return {
    ...
    search_text: [job.title, job.company, job.location, job.description, ...(job.tags || []), ...(job.skills || []), ...(job.niceToHave || [])].filter(Boolean).join(' '),
...
export async function syncJobsDb(jobs: Job[]): Promise<number> {
  ...
  try {
    ...
    await client.query(`UPDATE ${schema()}.vacancies SET active = FALSE, updated_at = NOW() WHERE active = TRUE AND sync_token <> $1`, [syncToken])
...
function sqlBuilder(): SqlBuilder {
  ...
  return { values, add(value) { values.push(value); return `$${values.length}` } }
}
...
function filteredWhere(query: JobQuery, sql: SqlBuilder): string {
  ...
  if (query.maxExperienceYears != null) parts.push(`(experience_min_years IS NULL OR experience_min_years <= ${sql.add(query.maxExperienceYears)})`)
  ...
  if (query.cities.length) {
    ...
    parts.push(`(LOWER(city) = ANY(${sql.add(cities)}::text[]) OR EXISTS (SELECT 1 FROM unnest(${sql.add(cities)}::text[]) term WHERE LOWER(location || ' ' || title) LIKE '%' || term || '%'))`)
  }
  ...
  if (query.skills.length) parts.push(`skills @> ${sql.add(query.skills.map((value) => value.toLocaleLowerCase('en')))}::text[]`)
  if (query.language) {
  ...
  if (query.excludeLanguages.length) parts.push(`NOT (language_keys && ${sql.add(query.excludeLanguages.map((value) => value.toLocaleLowerCase('en')))}::text[])`)
  return parts.join(' AND ')
}

function orderBy(sort: JobQuery['sort']): string {
  if (sort === 'oldest') return 'posted_at ASC, identity_key ASC'
  if (sort === 'title') return 'LOWER(title) ASC, posted_at DESC'
  if (sort === 'company') return 'LOWER(company) ASC, posted_at DESC'
  if (sort === 'salary') return 'salary_usd DESC NULLS LAST, posted_at DESC'
...
function emptyStats(): JobStats {
  return {
    ...
    byEmploymentKind: { fulltime: 0, parttime: 0, contract: 0, project: 0, freelance: 0, internship: 0, temporary: 0, volunteer: 0, seasonal: 0, unknown: 0 },
    experience: { knownCount: 0, medianYears: null, noExperience: 0, upToOne: 0, oneToThree: 0, threeToFive: 0, fivePlus: 0, unknown: 0 },
    byProfession: [], foreignerFriendly: 0, byLanguage: {}, topSkills: [], salaryTrend: [],
  }
}
...
export async function queryJobsDb(query: JobQuery): Promise<(JobResponse & { loadedSources: string[] }) | null> {
  ...
  try {
    ...
    const limit = sql.add(query.pageSize)
    const offset = sql.add((query.page - 1) * query.pageSize)
    const result = await db().query({
      text: `
        WITH filtered AS MATERIALIZED (
          SELECT * FROM ${schema()}.vacancies WHERE ${where}
        ), page_rows AS (
          SELECT data, ROW_NUMBER() OVER (ORDER BY ${orderBy(query.sort)}) AS page_order
          FROM filtered ORDER BY ${orderBy(query.sort)} LIMIT ${limit} OFFSET ${offset}
        ), grouped_source AS (
          SELECT source AS key, COUNT(*)::int AS count, COUNT(salary_usd)::int AS salary_count,
            COALESCE(percentile_cont(0.5) WITHIN GROUP (ORDER BY salary_usd) FILTER (WHERE salary_usd IS NOT NULL), 0)::float8 AS median
          FROM filtered GROUP BY source
        ), grouped_country AS (
          SELECT COALESCE(country, 'OTHER') AS key, COUNT(*)::int AS count, COUNT(salary_usd)::int AS salary_count,
            COALESCE(percentile_cont(0.5) WITHIN GROUP (ORDER BY salary_usd) FILTER (WHERE salary_usd IS NOT NULL), 0)::float8 AS median
          ...
          SELECT profession, COUNT(*)::int AS count, COUNT(salary_usd)::int AS salary_count,
            COALESCE(percentile_cont(0.5) WITHIN GROUP (ORDER BY salary_usd) FILTER (WHERE salary_usd IS NOT NULL), 0)::float8 AS median,
            (percentile_cont(0.5) WITHIN GROUP (ORDER BY experience_min_years) FILTER (WHERE experience_min_years IS NOT NULL))::float8 AS median_experience
          ...
          SELECT profession, 'country'::text kind, COALESCE(country, 'OTHER') key,
            ...
            COALESCE(percentile_cont(0.5) WITHIN GROUP (ORDER BY salary_usd) FILTER (WHERE salary_usd IS NOT NULL), 0)::float8 median
          ...
          SELECT *, ROW_NUMBER() OVER (PARTITION BY profession ORDER BY count DESC, salary_count DESC, median DESC) rank
          FROM profession_geo_raw WHERE salary_count > 0
        ), language_counts AS (
          SELECT item->>'language' AS key, COUNT(*)::int AS count FROM filtered, jsonb_array_elements(languages) item
        ...
        SELECT
          ...
          COALESCE((SELECT jsonb_agg(source ORDER BY source) FROM (SELECT DISTINCT source FROM ${schema()}.vacancies WHERE active = TRUE) active_sources), '[]'::jsonb) AS loaded_sources,
          (SELECT COUNT(*)::int FROM filtered) AS total,
          COALESCE((SELECT jsonb_agg(data ORDER BY page_order) FROM page_rows), '[]'::jsonb) AS jobs,
          COALESCE((SELECT jsonb_object_agg(source, count) FROM (SELECT source, COUNT(*)::int count FROM filtered GROUP BY source) s), '{}'::jsonb) AS sources,
          jsonb_build_object(
            'salary', COALESCE((SELECT jsonb_build_object('count', COUNT(salary_usd)::int, 'medianUsd', COALESCE(percentile_cont(0.5) WITHIN GROUP (ORDER BY salary_usd) FILTER (WHERE salary_usd IS NOT NULL), 0)::float8, 'avgUsd', COALESCE(AVG(salary_usd), 0)::float8, 'minUsd', COALESCE(MIN(salary_usd), 0)::float8, 'maxUsd', COALESCE(MAX(salary_usd), 0)::float8) FROM filtered), '{}'::jsonb),
            'bySource', COALESCE((SELECT jsonb_object_agg(key, jsonb_build_object('count', count, 'salaryCount', salary_count, 'medianUsd', median)) FROM grouped_source), '{}'::jsonb),
            'byCountry', COALESCE((SELECT jsonb_object_agg(key, jsonb_build_object('count', count, 'salaryCount', salary_count, 'medianUsd', median)) FROM grouped_country), '{}'::jsonb),
            'byWorkMode', COALESCE((SELECT jsonb_object_agg(key, count) FROM (SELECT COALESCE(work_mode, 'unknown') key, COUNT(*)::int count FROM filtered GROUP BY 1) x), '{}'::jsonb),
            'byRelocation', COALESCE((SELECT jsonb_object_agg(key, count) FROM (SELECT COALESCE(relocation, 'unknown') key, COUNT(*)::int count FROM filtered GROUP BY 1) x), '{}'::jsonb),
            'byEmploymentKind', COALESCE((SELECT jsonb_object_agg(key, count) FROM (SELECT COALESCE(employment_kind, 'unknown') key, COUNT(*)::int count FROM filtered GROUP BY 1) x), '{}'::jsonb),
            'experience', (SELECT jsonb_build_object('knownCount', COUNT(experience_min_years)::int, 'medianYears', percentile_cont(0.5) WITHIN GROUP (ORDER BY experience_min_years) FILTER (WHERE experience_min_years IS NOT NULL), 'noExperience', (COUNT(*) FILTER (WHERE no_experience OR experience_min_years = 0))::int, 'upToOne', (COUNT(*) FILTER (WHERE NOT no_experience AND experience_min_years > 0 AND experience_min_years <= 1))::int, 'oneToThree', (COUNT(*) FILTER (WHERE NOT no_experience AND experience_min_years > 1 AND experience_min_years <= 3))::int, 'threeToFive', (COUNT(*) FILTER (WHERE experience_min_years > 3 AND experience_min_years <= 5))::int, 'fivePlus', (COUNT(*) FILTER (WHERE experience_min_years > 5))::int, 'unknown', (COUNT(*) FILTER (WHERE experience_min_years IS NULL))::int) FROM filtered),
            'byProfession', COALESCE((SELECT jsonb_agg(jsonb_build_object(
              'profession', gp.profession, 'count', gp.count, 'salaryCount', gp.salary_count,
              'medianUsd', gp.median, 'medianExperienceYears', gp.median_experience,
              'geographies', COALESCE((SELECT jsonb_agg(jsonb_build_object('kind', kind, 'key', key, 'count', count, 'salaryCount', salary_count, 'medianUsd', median) ORDER BY rank) FROM profession_geo WHERE profession = gp.profession AND rank <= 6), '[]'::jsonb)
            ...
            'topSkills', COALESCE((SELECT jsonb_agg(jsonb_build_object('skill', key, 'count', count)) FROM skill_counts), '[]'::jsonb),
            'salaryTrend', COALESCE((SELECT jsonb_agg(jsonb_build_object('postedAt', posted_at, 'salaryUsd', salary_usd, 'country', country, 'city', city, 'title', title, 'profession', profession, 'experienceYears', experience_min_years) ORDER BY posted_at) FILTER (WHERE salary_usd IS NOT NULL) FROM filtered), '[]'::jsonb)
          ) AS stats
      `,
      values: sql.values,
    })
    ...
    if (!row) return null
    if ((Number(row.database_total) || 0) === 0) return null
    const stats = { ...emptyStats(), ...(row.stats || {}) } as JobStats
    ...
    return { jobs: row.jobs || [], total: Number(row.total) || 0, page: query.page, pageSize: query.pageSize, sources: row.sources || {}, stats, loadedSources: row.loaded_sources || [] }
  } catch (error) {
...
export async function getJobByPublicIdDb(publicId: string): Promise<Job | null> {
  ...
  try {
    await ensureSchema()
    const result = await db().query(`SELECT data FROM ${schema()}.vacancies WHERE active = TRUE AND public_id = $1::bigint LIMIT 1`, [publicId])
