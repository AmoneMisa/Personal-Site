...
function itemToProfile(item: LinkedInItem, target: LinkedInTarget, checkedAt: string): CvProfile | null {
  ...
  return normalizeCandidate({
    ...
    country: target.country,
    name: author && author.length <= 100 ? author : '',
    role,
...
async function fetchTarget(target: LinkedInTarget): Promise<{ profiles: CvProfile[]; fetched: number }> {
  ...
  if (!response.ok || body.ok === false) {
    throw new Error(body.error || `LinkedIn candidate fetcher -> HTTP ${response.status}`)
  }
  ...
  return {
    ...
    profiles: items
      .map((item) => itemToProfile(item, target, checkedAt))
      .filter((item): item is CvProfile => Boolean(item)),
  }
}
...
export async function refreshHiringLinkedInSource(handle: string): Promise<{ fetched: number; candidates: number; stored: number } | null> {
  ...
  const checkedAt = new Date().toISOString()
  const base = {
    handle: `linkedin:${target.key}`,
    key: `linkedin:${target.key}`,
    label: target.label,
    country: target.country,
    pages: 1,
    duplicate: 0,
  ...
  try {
    ...
    const diagnostic: WebSourceDiagnostic = {
      ...
      newestActivityAt: times.length ? times[times.length - 1]! : null,
