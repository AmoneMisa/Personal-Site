import { extractJobStructuredField } from '@whiteslove/parsing-lexicon/hiring-source-semantics'
...
function urlsFromText(text: string): string[] {
  ...
  return matches.map(normalizeExternalUrl).filter((url): url is string => !!url)
}
